...
int main(int argc, char** argv){
    ...
    char opt;
    while( (opt = getopt(argc, argv, "i:s:c:l:m:e:f:n:p:d:o:O:v")) != -1){
        switch(opt){
            case 'i':
    ...
    if(!flag_input_file || !flag_labels_file || !flag_edge_file || !flag_snp_file || !flag_map_file || !flag_covar_file || !((bool)flag_out)){
    ...
    Data data = Data(data_file, labels_file, snp_file, covar_file, map_file, edge_file);
    if(n_perm > 0){
        HOGImineWY algo = HOGImineWY(&data, target_fwer, max_interval_dim, n_perm);
        cout << "Init: " << second() - tic << " seconds " << endl;
        algo.process_edges();
        algo.write_significant(out_file, print_pv, flag_out==2);
