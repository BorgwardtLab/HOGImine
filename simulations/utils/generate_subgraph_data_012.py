...
def main():
  ...
  for sim_id in range(0, N_SIMULATIONS):
      ...
      # distr: 0: 0.980 1: 0.012 2: 0.008
      data = np.random.binomial(1, 0.02, (n_samples, n_features))*(1+np.random.binomial(1, 0.4, (n_samples, n_features)))
      #print(data)
      ...
      encoding_sig = np.any(data[:, intervs_true] >= 2, axis = 1)
      encoding_conf = np.any(data[:, intervs_conf] >= 1, axis = 1)
      ...
      data1 = (data == 1)
      data2 = (data == 2)
      np.savetxt(f'{out_pref}_X.txt', np.transpose(data0), fmt='%d')
      np.savetxt(f'{out_pref}_X1.txt', np.transpose(data1), fmt='%d')
      np.savetxt(f'{out_pref}_X2.txt', np.transpose(data2), fmt='%d')
