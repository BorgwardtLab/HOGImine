...
public:
    ...
    CMHHandler cmh;
    vector<IntervalMap> gene_intervals;
    ...
    void process_edges(){
        ...
        for(vector<int> edge : data->edgeList){
            vector<IntervalMap> intv_maps(edge.size());
            vector<int> genes(edge.size());
            for(int i=0; i<edge.size(); i++){
                intv_maps[i] = gene_intervals[edge[i]];
                genes[i] = edge[i];
            }
    ...
    void process_genes(){
        gene_intervals = vector<IntervalMap>(data->n_genes);
        for(int i=0; i<data->n_genes; i++){
            gene_intervals[i] = make_gene_intervals_bfs(i);
        }
        //cout << cnts << endl;
    }

    IntervalMap make_gene_intervals_bfs(int gene){ // this can return only closed patterns, dfs cannot
    ...
    void test_interval_combinations_helper(vector<IntervalMap>& gene_itvl, vector<int>& gene_ids, ofstream& out_file){
                                            ...
                                            ofstream& out_file, int depth, int maxdepth)
    {
        if(depth == maxdepth){
            ...
            double minpv = 1.0;
            cnt_process++;
            int cas = cmh.process_pattern(support, &pvalue, &minpv);
            if(cas == 0){ // testable
    ...
    void create_len1_intervals(FrontierMap& frontier, vector<IntervalMap>& gene_itvl, vector<int>& gene_ids,
                                vector<pair<int, int>>& intervals, Bitset& support,int depth, int maxdepth)
    {
        if(depth == maxdepth){
            double pvalue = 1.0;
            double minpv = 0.0;
            ...
            if(cas < 2){ // not prunable
                ...
                for(int i=0; i<maxdepth; i++) key[i] = intervals[i].first*10000 + intervals[i].second; // encode as start*10000+len
        ...
        for(int start=0; start<n_snps; start++){
            if(depth == 0){
                support = gene_itvl[depth][start][0];
            }
            else{
                support = support.bitwise_or(gene_itvl[depth][start][0]);
            }
            intervals[depth] = make_pair(start, 1);
    ...
    vector<tuple<vector<int>, double>> test_interval_combinations_bfs(vector<IntervalMap>& gene_itvl, vector<int>& gene_ids){
        int n_genes = gene_itvl.size();
        FrontierMap frontier;
        vector<tuple<vector<int>, double>> out;

        vector<pair<int, int>> intv(n_genes);
        Bitset supp;
        ...
        while(1){
            ...
            for(auto& pattern : frontier){
                ...
                int len0 = key[0]%10000;

                for(int i=0; i<gene_itvl.size(); i++){ // gene to enlarge interval
                    int keyi = key[i];

                    int starti = keyi/10000; int leni = keyi%10000;
                    if(i > 0 && len0 > 1) continue; // this is to avoid generating the same pattern multiple times, but still generate all patterns
                    if( gene_itvl[i][starti][leni].n == 0)continue; // check if [start, len+1] is in gene_itvl

                    key[i] += 10000; // increase by 1 starting position and keep len
    ...
    void write_significant(string output_filename, bool print_pv){
        ...
        if(print_pv) {
            ...
            for(auto& a : testable_patterns){
                ...
                for(int i=0; i<pat.size()/2; i++){
                    int ii = pat[i];
                    pv_file << "," << data->gene_names[pat[pat.size()/2 + i]] << "," << ii/10000 << "_" << ii%10000;
                }
                pv_file << endl;
            }
            pv_file.close();
        ...
        for(auto& a : testable_patterns){
            ...
            if(pv <= thr){
                ...
                for(int i=0; i<pat.size()/2; i++){
                    int ii = pat[i];
                    sign_file << "," << data->gene_names[pat[pat.size()/2 + i]] << "," << ii/10000 << "_" << ii%10000;
                }
                cnt_signif++;
                sign_file << endl;
            }
